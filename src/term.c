...
static int cellWGlobal = 9, cellHGlobal = 18;
static int originColGlobal = 1, originRowGlobal = 1;
...
bool term_init(int *outW, int *outH)
{
    ...
    *outH = py & ~1;
    ...
    /* alt screen, hide cursor, clear, enable SGR mouse clicks and motion */
    write_str("\x1b[?1049h\x1b[?25l\x1b[?1003h\x1b[?1006h\x1b[2J\x1b[H");
    return true;
}
...
static void restore_terminal(void)
{
    ...
    write_str("\x1b[?1003l\x1b[?1000l\x1b[?1006l\x1b[?25h\x1b[?1049l");
...
static int parse_sgr_mouse(void)
{
    ...
    if (sscanf(buf, "%d;%d;%d", &button, &col, &row) != 3)
        return -1;
    int px = (col - originColGlobal) * cellWGlobal;
    int py = (row - originRowGlobal) * cellHGlobal;
    if (G.W > 0) px = (int)clampf((float)px, 0.0f, (float)(G.W - 1));
    if (G.H > 0) py = (int)clampf((float)py, 0.0f, (float)(G.H - 1));
    G.mouseX = px;
    G.mouseY = py;
    if (final == 'M' && (button & 3) == 0 && !(button & 32))
        return KEY_MOUSE;
